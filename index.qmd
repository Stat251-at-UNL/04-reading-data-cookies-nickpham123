...
The scatter plot shows that most ingredients are used in pretty small quantities (under 5 cups), and ratings are spread all over the place regardless of quantity, so there doesn't seem to be much of a relationship between the two. The histogram backs this up, showing that the vast majority of quantities are clustered near zero with a few extreme outliers pushing up to 48 cups. It would be interesting to dig deeper into those outliers and see if specific ingredients at certain quantities tend to show up in higher-rated recipes